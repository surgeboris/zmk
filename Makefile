...
build_right: own start
	docker exec -w /zmk/app zmk west build -p -d build/right -b nice_nano_v2 -- -DSHIELD="corne_right nice_view_adapter nice_view"; make own_back
clean: own_back
	rm -rf ./app/build
clean_all: clean
...
init: own
	docker run -d --name zmk -v .:/zmk -w /zmk docker.io/zmkfirmware/zmk-dev-arm:3.5-branch tail -f ./Makefile && docker exec zmk /bin/bash -c "west init -l app" && docker exec zmk /bin/bash -c "west update"
own:
	sudo chown -R root:root .
own_back:
	sudo chown -R $$(id -u):$$(id -g) .
start:
	sudo rc-service docker start && docker start zmk
